...
def save_checkpoint(queue, args):
    ...
    # if md.model_type == 'gemma':
    #     sys.argv += ["--kv_channels", str(256)]
    ...
    if md.model_type == 'GPT':
        ...
        margs.model_type = ModelType.encoder_or_decoder
    elif md.model_type in {'falcon', 'llama', 'llama2', 'codellama', 'mistral', 'qwen', 'gemma'}:
    ...
    if md.true_vocab_size is not None:
        ...
        #     # Random uniform initialization in range [-a, a] for padding
        #     a = 0.02  # for example, adjust as needed
        #     random_padding_embed = (2 * a * torch.rand(padding_size, orig_word_embed.shape[1]) - a).to(orig_word_embed.device)

        #     full_word_embed = torch.cat((orig_word_embed, random_padding_embed))

        #     if not md.tie_embed_logits:
        #         random_padding_head = (2 * a * torch.rand(padding_size, lm_head.shape[1]) - a).to(lm_head.device)
        #         full_lm_head = torch.cat([lm_head, random_padding_head])
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer in range(len(models[0].language_model.encoder.layers)):
            ...
            if md.use_bias or md.model_type == 'qwen':
            ...
            else: # gemma and qwen follow this path
