...
class ParallelAttention(MegatronModule):
    ...
    def __init__(self,
        ...
        use_qkv_bias = args.use_bias or args.model_name=='qwen'
        ...
        if self.position_embedding_type == PositionEmbeddingType.rotary:
            self.freqs_cis = precompute_freqs_cis(
                ...
                dim=args.kv_channels if args.model_name=='gemma' else args.hidden_size // args.num_attention_heads,
                end=self.seq_length,
    ...
    def forward(self,
        ...
        if self.attention_type == AttnType.self_attn:
            ...
            # heads/parallel * 3 * hiddensize/heads
            ...
            qkv = mixed_x_layer.reshape(sq, b, -1, self.num_attention_heads // self.num_attention_heads_kv + 2, self.hidden_size_per_attention_head)
        ...
        # ==================================
        # Rotary embeddings
        # NOTE: need to be modified by varlen
        # ==================================
        if self.position_embedding_type == PositionEmbeddingType.rotary:
        ...
        if not self.use_flash_attn:
            ...
            if (attention_mask is not None) and (self.intra_doc):
                cum_len, max_len, min_len = attention_mask
                flash_attn_extra_kwargs['cu_seqlens_q'] = cum_len
                flash_attn_extra_kwargs['cu_seqlens_k'] = cum_len
                flash_attn_extra_kwargs['max_seqlen_q'] = max_len
                flash_attn_extra_kwargs['max_seqlen_k'] = max_len
                seqlen, bsz, _, _ = query_layer.size()
                q, k, v = [rearrange(x, "s b n h -> b s n h").contiguous()
                    for x in (query_layer, key_layer, value_layer)]
                q, k, v = [rearrange(x, "b s n h -> (b s) n h").contiguous()
                    for x in (q, k, v)]
            else:
                q, k, v = [rearrange(x, "s b n h -> b s n h").contiguous()
                    for x in (query_layer, key_layer, value_layer)] 
            ...
            if (attention_mask is not None) and (self.intra_doc):
                _, nheads, headdim = context_layer.size()
                context_layer = context_layer.view(bsz, seqlen, nheads, headdim) #rearrange(context_layer, 'b s n h -> s b (n h)').contiguous()
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(self,
        ...
        self.use_qkv_bias = args.model_name=='qwen'
